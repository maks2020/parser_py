...
def make_url_things_list(sources_html):
    ...
    things_list = soup.select('css selector')
    url_things_list = []
...
name_html = config_parse['name_html']  # name parser
catalog_results = config_parse['catalog_results']
cat_file_html = config_parse['cat_file_html']  # source html common catalog
#path url file pages of things
path_things_url_file = config_parse['path_things_url_file'] = (
    config_parse['path_output_file'] + 'thing_url_' + name_html + '.txt')
name_data_file = config_parse['name_data_file'] # name config file
